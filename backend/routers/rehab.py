...
router = APIRouter(prefix="/api/rehab", tags=["rehab"])
...
class FeedbackResponse(BaseModel):
    ...
    encouragement: str
    processing_ms: int
    zone_scores: Optional[dict] = None
...
# ---------------------------------------------------------------------------
# Helper: call Gemini Vision with fallback
# ---------------------------------------------------------------------------
def _call_gemini(image_bytes: bytes, prompt: str) -> dict:
    ...
    # Convert image to JPEG bytes for inline_data
    ...
    raw, model_used = generate_with_fallback(content=[prompt, image_part])
